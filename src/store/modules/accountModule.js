...
const actions = {
   ...
   * 是否处于登录状态
   */
  [storeTypes.ACCOUNT_IS_LOGIN]({commit}, param) {
    var type = 'GET'
    var url = 'mobile/home/mobileHome/isLogin'
   ...
   * 获取继续教育年度
   */
  [storeTypes.ACCOUNT_GET_EDUCATION_YEAR]({commit}, param) {
    var type = 'GET'
    var url = 'mobile/user/userOrder/getAllOrderYear'
    return request(type, param, url)
      .then(function (data) {
        return (data)
      }, function (reason) {
   ...
   * 获取继续教育年度学分情况
   */
  [storeTypes.ACCOUNT_GET_EDUCATION_CREDIT_SITUATION]({commit}, param) {
    var type = 'GET'
    var url = 'mobile/user/userOrder/yearLearnSituation'
    return request(type, param, url)
      .then(function (data) {
        return (data)
      }, function (reason) {
  ...
  [storeTypes.ACCOUNT_REGISTER_REQUEST]({commit}, params) {
    let type = 'post'
    let url = 'mobile/home/mobileRegister/userRegisterActivate'
  ...
  [storeTypes.ACCOUNT_CHANGE_TELEPHONENUM]({commit}, param) {
    var type = 'POST'
    var url = 'mobile/home/mobileRegister/updateUserPhoneNum'
    return request(type, param, url)
      .then(function (data) {
        if (data.head.code === storeTypes.NETWORK_RESULT_SUCCESS) {
          return (data)
  ...
  // 校验帐号合法性
  [storeTypes.CHECK_ACCOUNT]({commit}, params) {
    let type = 'POST'
    let url = 'mobile/home/mobileRegister/validateLoginAccount'
    return request(type, params, url).then((data) => {
      return data
    }, (reason) => {
      return ({head: {code: 700, message: '请求失败'}})
  ...
  [storeTypes.GET_SMS_CODE]({commit}, params) {
    let type = 'GET'
    let url = 'mobile/home/mobileRegister/sendPhoneValidateCode'
    return request(type, params, url).then((data) => {
      return data
