<template>
  <div class="weui-panel hb-panel bor-b20" @click="cellClickAction">
    ...
    <!--<div class="to c6 fs28" style="height: .45rem;line-height: .45rem"><span class="label-company">机构</span>{{item.unitName}}</div>-->
    <!--</div>-->
    ...
    <order-list-sub-cell v-for="subItem in item.subOrderShow" :key="subItem.subOrderNo"
                         ...
                         @on-subChangeClassRecord="changeClassRecordAction(subItem)"></order-list-sub-cell>
    ...
    <div class="weui-panel__op bor-t" ref="bottomBar" v-if="isShowBottomBar()"
      ...
      <div class="hb-btn-s" @click="buttonAction('cancelOrder', $event)"
           v-if="isShowCancelOrderButton(this.item) && item.paymentChannel !== 'COLLECTIVE' && item.paymentChannel !== 'PRESENT'">
        取消订单
      </div>
      <div class="hb-btn-s current" @click="buttonAction('goToPay', $event)"
           v-if="isShowCancelOrderButton(this.item) && item.paymentChannel !== 'PRESENT'">立即支付
      </div>
    ...
    methods: {
      ...
      isShowBottomBar () {
        ...
        if (this.item.status === 7) {
          ...
          if (this.isShowSeeRefundButtton(this.item) || this.isShowChangeClassRecordButton(this.item) || this.isShowRelevanceOrder(this.item) || this.isShowCancelOrderButton(this.item) || this.isShowcompleteDistributionInfoButtton(this.item)) {
            return true
          } else {
            return false
          }
