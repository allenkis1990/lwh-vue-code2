  ...
  <div class="contain hb-box">
    <div class="hb-box-bd">
          ...
          <div class="weui-cell pbr0">
            <div class="weui-cell__bd">
              <div class="hb-btn-s w1"
                   ...
                   v-for="(item,index) in invoiceTypeItemsArray">{{item.invoiceTypeTitle}}
              </div>
            </div>
          </div>
          <div class="p20 fs26 c9" v-if="selectedInvoiceType ===1">纸质发票开出以后，我们将会以快递的形式将发票寄送到您制定的地址<span class="ci">（快递费到付）</span>
          </div>
          <div class="p20 fs26 c9" v-if="selectedInvoiceType ===2">电子发票即增值税普通发票的电子版本，可支持报销入账，其法律效力、基本用途及使用规定和纸质发票相同。
          </div>
          <div class="p20 fs26 c9" v-if="selectedInvoiceType ===3">我公司依法开具发票，请您按照税法规定使用发票。</div>
          <div class="p20 fs26 c9" v-if="selectedInvoiceType ===4">非税务票是财政投放的票据，行政事业单位收取行政事业性收费开具的票据，是正式票据，可以作为报销凭证。
          </div>
        </div>
      </div>
      <div class="weui-panel hb-panel bor-b20"
           v-if="selectedInvoiceType ==1 || selectedInvoiceType ==2 || selectedInvoiceType ==4">
      ...
      <div class="weui-panel hb-panel"
           v-show="(selectedInvoiceTitleType ==2 && (selectedInvoiceType ==1 || selectedInvoiceType == 2 || selectedInvoiceType == 4))">
        <div class="weui-cells weui-cells_form">
          <div class="weui-cell">
            <div class="weui-cell__hd">
        ...
        <div class="c9 fs26 p20" v-show='!(selectedInvoiceType == 4)'>
          按照国税总局公告，自2017年7月1日起，企业索取的增值税发票需填写纳税人识别号，不符合规定的发票，不得作为合法的税收凭证。
        </div>
      </div>
      <!--/单位-->
      <!--增票资质-->
...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      this.$store.commit(storeTypes.ORDER_SAVE_SELECT_CREDIT, options.selectCredit)
...
//      进来没填用户名字的时候默认用userName
      if (this.selectedInvoiceTitleType === 1) {
        if (!this.personName) {
          this.personName = this.$store.state.accountModule.userInfo.userName
        }
      }
    },
    methods: {
