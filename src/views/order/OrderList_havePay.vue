  ...
  <div>
    <scroller lock-x scrollbar-y :height="scrollerHeight" use-pulldown use-pullup @on-pullup-loading="loadMoreData"
      ...
      <div>
        ...
        <order-list-cell v-for="item in items" :item="item" :key="item.orderNo" @on-cellClick="cellClickAction"
                         @on-changeClassRecord="changeClassRecordAction" @on-relevanceOrder="relevanceOrderAction"
                         @on-seeRefund="seeRefundAction"
                         @on-completeDistributionInfo="completeDistributionInfoAction"></order-list-cell>
      </div>
      <div slot="pulldown" class="xs-plugin-pulldown-container xs-plugin-pulldown-down"
        ...
        <span class="pulldown-arrow" v-show="scroller.pulldownStatus === 'down' || scroller.pulldownStatus==='up'"
              :class="{'rotate': scroller.pulldownStatus === 'up'}" style="font-size: 16px;font-weight: bold;">↓</span>
  ...
  export default {
    ...
    methods: {
      ...
      relevanceOrderAction (item) {
        let param = {
          orderNo: item.subOrderList[0].subOrderNo,
          type: 'ASSOCIATEORDER'
        }
      ...
      completeDistributionInfoAction (item) {
        ...
        this.push({ path: '/completeOrder', query: { path: '/demo/orderList' } })
      },
      /**
       * cell点击事件，跳转详情页
       */
