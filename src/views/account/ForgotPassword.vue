  ...
  <div class="contain">
    ...
    <div class="hb-header-box">
      ...
      <div class="header-right">
        <a class="header-txt"><span @click="() => {this.push('/loginByPassword')}">登录</span>/<span
          @click="() => {this.push('/regiter')}">注册</span></a>
      </div>
    </div>
    <!--标题栏-->
    <div class="hb-form-cells mt150">
      <div class="weui-cells weui-cells_form">
        <div class="weui-cell">
          <div class="weui-cell__hd"><span class="weui-label">证件号</span></div>
          <div class="weui-cell__bd"><input type="text" placeholder="请输入证件号" class="weui-input"
                                            v-model="identityCardNo"/></div>
        </div>
        <div class="weui-cell">
          <div class="weui-cell__hd"><span class="weui-label">姓名</span></div>
          <div class="weui-cell__bd"><input type="text" placeholder="请输入姓名" class="weui-input" v-model="userName"/>
          </div>
        </div>
      </div>
      ...
      submit () {
        ...
        this.$store.dispatch(storeTypes.ACCOUNT_RESET_PASSWORD_REQUEST, params).then(data => {
          ...
          if (data.head.code !== storeTypes.NETWORK_RESULT_SUCCESS) {
            if (data.head.code == 805) {
              this.$vux.confirm.show({
                title: '提示',
                content: '当前证件号未注册，请先注册',
                confirmText: '前往注册',
                cancelText: '取消',
                onConfirm: function () {
                  context.push('/regiter')
                }
              })
              return
            } else {
